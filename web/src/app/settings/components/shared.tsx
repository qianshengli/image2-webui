    ...
    <>
      {items.map((item, index) => (
        <div key={`${item.title}-${index}`} className={index === 0 ? "" : "mt-2"}>
          <span className="font-semibold text-stone-800 dark:text-[var(--studio-text-strong)]">{item.title}：</span>
          {item.body}
        </div>
      ))}
    </>
...
export function ConfigSection({
  ...
  return (
    <Card className="border-stone-200 bg-white shadow-[0_18px_48px_rgba(15,23,42,0.05)] dark:border-[var(--studio-border)] dark:bg-[var(--studio-panel-soft)]">
      <CardContent className="space-y-4 p-4 sm:space-y-5 sm:p-6">
        <div className="flex flex-col gap-2 sm:gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <div className="text-[15px] font-semibold tracking-tight text-stone-900 dark:text-[var(--studio-text-strong)] sm:text-base">{title}</div>
            <p className="mt-0.5 text-xs leading-5 text-stone-500 dark:text-[var(--studio-text)] sm:mt-1 sm:text-sm sm:leading-6">{description}</p>
          </div>
...
export function Field({
  ...
  return (
    <label className={fullWidth ? "space-y-1.5 md:col-span-2 sm:space-y-2" : "space-y-1.5 sm:space-y-2"}>
      <div className="text-[13px] font-medium text-stone-700 dark:text-[var(--studio-text-strong)] sm:text-sm">
        <LabelWithHint label={label} tooltip={tooltip ?? hint} />
      </div>
      <div>{children}</div>
      <div className="hidden text-xs leading-5 text-stone-400 dark:text-[var(--studio-text-muted)] sm:block">{hint}</div>
    </label>
  );
}
...
export function ToggleField({
  ...
  return (
    <div className="rounded-2xl border border-stone-200 bg-stone-50/70 p-3 dark:border-[var(--studio-border)] dark:bg-[var(--studio-panel-muted)] md:col-span-2 sm:p-4">
